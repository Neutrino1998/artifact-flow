...
![Frontend_01](docs/assets/frontend_screenshot_01.png)
![Frontend_02](docs/assets/frontend_screenshot_02.png)
...
- **双Artifact架构**: 任务计划与结果文稿分离，支持版本追溯
- **多智能体协作**: Lead/Search/Crawl 专业分工，Pi-style 扁平循环编排
- **Agent 即配置**: Agent 定义为 MD 文件（YAML frontmatter + 角色提示词），无需写代码即可自定义 Agent
- **单轮多工具调用**: Agent 单轮可调多个工具，串行执行，权限中断自然嵌入
- **执行中消息注入**: 用户可在执行过程中追加指令，turn boundary 非阻塞 drain 到 LLM context
- **事件溯源持久化**: MessageEvent append-only 事件表记录完整执行过程，支持历史回放和可观测性
- **流式响应**: 实时输出思考过程和工具调用，SSE 推送
- **权限控制**: 工具分级权限（AUTO/CONFIRM），支持中断确认后恢复执行
- **分支对话**: 从任意历史节点创建新分支
- **持久化存储**: SQLite + WAL + 乐观锁，服务重启数据不丢失
...
├─ StreamManager (事件缓冲队列)       ├─ ConversationManager
├─ TaskManager (任务生命周期)         ├─ ArtifactManager
├─ agents config (只读 Dict)         ├─ ExecutionController
└─ tools (只读 Dict)                 └─ MessageEventRepository
```

**设计原则：**

- **全局单例只做协调**：`TaskManager` 管理协程引用、并发上限（`asyncio.Semaphore`）、interrupt（`asyncio.Event`）和消息注入；`StreamManager` 管理事件缓冲队列（`asyncio.Lock` 保护字典操作）；`DatabaseManager` 管理连接池。它们不持有业务状态。
