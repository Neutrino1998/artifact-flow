...
def _configure_paths(monkeypatch, tmp_path: Path, instance_id: str) -> tuple[Path, Path, Path, Path]:
    log_root = tmp_path / "logs"
    obs_root = tmp_path / "observability"
    marker = tmp_path / "autoheal.jsonl"
    ...
    monkeypatch.setattr(config, "OBS_AUTOHEAL_MARKER_PATH", str(marker))
    return (
...
def test_reads_error_wedge_metrics_and_autoheal(monkeypatch, tmp_path):
    instance_id = "backend-1"
    error_log, loop_log, metrics_log, marker = _configure_paths(
    ...
    assert by_type["autoheal"]["reason"] == "unhealthy"
...
def test_limit_is_applied_after_newest_first_sort(monkeypatch, tmp_path):
    ...
    error_log, _, _, _ = _configure_paths(monkeypatch, tmp_path, instance_id)
    error_log.parent.mkdir(parents=True)
    error_log.write_text(
