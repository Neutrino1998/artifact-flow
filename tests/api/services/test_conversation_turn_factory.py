"""
Tests for conversation_turn_factory event sanitization and stream forwarding.
...
from api.services.conversation_turn_factory import run_and_push, sanitize_error_event
...
def test_debug_keeps_error_and_injects_request_id(monkeypatch, with_request_id):
    monkeypatch.setattr("api.services.conversation_turn_factory.config.DEBUG", True)
    out = sanitize_error_event(_error_event())
