...
def test_release_script_is_build_only_and_emits_strict_manifest() -> None:
    ...
    # Every macOS-authored tar layer must drop provenance xattrs. Cover the
    # config archive, deploy staging/final archives, and outer transport.
    assert release.count("COPYFILE_DISABLE=1 tar --no-xattrs") == 4
...
def test_ansible_renders_the_snippets_caddy_imports_and_leaves_state_order_to_afctl() -> None:
    ...
    assert 'async: "{{ af_ready_timeout_seconds }}"' in playbook
    assert "poll: 1" in playbook
