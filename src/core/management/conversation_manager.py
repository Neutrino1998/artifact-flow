...
class ConversationResourceNotFoundError(NotFoundError):
    """Application-facing not-found error for conversation use cases."""
...
class ConversationManager:
    ...
    async def list_admin_conversations(
        ...
        *,
        ...
        user_names = await UserRepository(repo.session).display_names(user_ids)
    ...
    async def get_admin_conversation_events(
        ...
        owner_names = await UserRepository(repo.session).display_names(
            {conv.user_id} if conv.user_id else set()
        )
        owner_display_name = owner_names.get(conv.user_id) if conv.user_id else None
