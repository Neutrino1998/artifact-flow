...
from repositories.department_access_repo import (
...
class DepartmentAccessManager:
    ...
    async def get_department_access(self, dept_id: str) -> dict:
        ...
        skills = [
            self._serialize_skill(skill, departments_by_id)
            for skill in await self._repo.list_skill_access(dept_ids)
        ]
        units = [
            self._serialize_unit(unit, departments_by_id)
            for unit in await self._repo.list_unit_access(dept_ids)
        ]
    ...
    async def put_unit_rule(self, dept_id: str, unit_name: str) -> None:
        ...
        async def _exists() -> bool:
            return await self._repo.has_unit_rule(dept_id, unit_name)
    ...
    async def _commit_idempotent_put(
        self, exists_check: Callable[[], Awaitable[bool]]
    ) -> None:
        try:
            await self._repo.commit()
        except IntegrityError:
            await self._repo.rollback()
            # Concurrent PUT of the same natural-key row is a successful
            # idempotent retry. Other integrity failures must still surface.
            if await exists_check():
                return
            raise

    def _serialize_skill(
        self,
        skill: SkillAccessProjection,
        ...
        return {
            ...
            "direct_rule": skill.direct_rule,
            "inherited_rule": _serialize_inherited_rule(inherited_from),
            "effective_allowed": _effective_allowed(
                skill.visibility,
                _matched(skill.direct_rule, skill.inherited_department_id),
            ),
        }

    def _serialize_unit(
        self,
        unit: UnitAccessProjection,
        departments_by_id: dict[str, Department],
    ) -> dict:
        inherited_from = departments_by_id.get(unit.inherited_department_id)
        return {
            "name": unit.name,
            "kind": unit.kind,
            ...
            "direct_rule": unit.direct_rule,
            "inherited_rule": _serialize_inherited_rule(inherited_from),
            "effective_allowed": _effective_allowed(
                unit.visibility,
                _matched(unit.direct_rule, unit.inherited_department_id),
            ),
        }
...
def _matched(direct_rule: bool, inherited_department_id: Optional[str]) -> bool:
    return direct_rule or inherited_department_id is not None
