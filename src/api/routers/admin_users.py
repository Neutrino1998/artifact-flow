...
User-management endpoints (admin-only). Mounted at /api/v1/admin so paths are:
- POST   /api/v1/admin/users                       - 创建用户
- GET    /api/v1/admin/users                       - 用户列表
- GET    /api/v1/admin/users/bulk-impact           - 批删前的影响数据
- POST   /api/v1/admin/users/bulk-action           - 批量动作（disable/enable/delete/set_department）
- POST   /api/v1/admin/users/bulk-import           - CSV 批量导入
- GET    /api/v1/admin/users/{user_id}             - 单查
- GET    /api/v1/admin/users/{user_id}/impact      - 硬删前的影响数据
- PUT    /api/v1/admin/users/{user_id}             - 更新
- DELETE /api/v1/admin/users/{user_id}             - 硬删

Self-facing endpoints (me / login / password) stay in routers/auth.py.
"""
...
# ============================================================
#
# 注册顺序：放在 /{user_id} 系列之前，避免被解析为 user_id="bulk-import"。
...
# ============================================================
# Per-user endpoints (must register after /users/bulk-* to avoid
# /{user_id} swallowing "bulk-impact" / "bulk-action" / "bulk-import")
# ============================================================
