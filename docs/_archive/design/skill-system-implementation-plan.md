...
> 状态:规划完成;A 合 main,B 进行中(B-1 已合 main,B-2 待开工)
> 起草:2026-06-16 · 最后更新:2026-06-25
> 前序产物:
...
- **当前**:**A 合 main;B 进行中 —— B-1(DB 地基 + 通用 reconciler)已合 main(落地见 Phase B「进展」)。** B 内部分 4 片增量合 main(B-1 DB 模型+reconciler+snapshot 读侧 → B-2 EffectiveToolset resolver+引擎切快照+decision-11 改写 → B-3 deferred+search_tools+catalog 挪位 → B-4 provider 缝+前端 CRUD/agent_unit 绑定 UI)。**开工细化见同目录 `skill-system-phase-b-design.md`**。架构主分叉已**全部收口** —— 披露归工具层 / 可见性两正交字段 / 依赖三层离线 / 导入硬软双门 / read+mount 零新机制 / external 工具·tool-set·mcp·agent 全 DB 物化(通用 reconciler)/ 部门授权两张 FK 表 / 工具权限两正交轴(等级 + 成员)—— **逐条见「已锁定的决策」+「数据模型总览」,此处不复述**。
- **下一步**:**B-2(`EffectiveToolset` resolver + 引擎切 DB 快照 + decision-11 的 agent MD 重写)** —— B-1 已物化注册表但引擎尚未消费;B-2 是把物化接进引擎、且 B 内唯一改行为的一片(成员态/等级分离、等级移到工具定义、旧 `auto/confirm` 字面量 loud-fail)。
...
| B | 工具渐进式披露(tool-set DB 模型[迁出 config]+ `search_tools` 内建工具;MCP 适配缝) | **进行中**(B-1 合 main;B-2/3/4 待开工) |
...
**进展**:开工细化见同目录 `skill-system-phase-b-design.md`(B-1 是 write-only 物化、引擎不动;B-2 才把物化接进引擎 + 做 decision-11 重写)。
