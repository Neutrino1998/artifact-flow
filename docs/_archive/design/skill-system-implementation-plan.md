...
- **可见 skill** = `skill` 的 visibility/default + `user_skill(U)` 覆盖 + `department_skill_rule(D)`;**全 agent 可见、不分 agent**(效果按各 agent 宇宙收窄,决策 11)。
