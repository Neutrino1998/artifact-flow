...
- **当前**:**规划完成,未开工。** 沙盒主线(底座)已收官。架构主分叉已收口(披露归工具层 / 可见性=visibility+default_enabled+稀疏覆盖 / 依赖三层离线 / 导入硬软双门 / read+mount 复用零新机制 / external 工具/tool-set/mcp + agent 全 DB 物化[通用 config-seed→DB reconciler、skill/tool/agent 一套种子机制、config 仅作启动种子、注册表每 turn 一次 DB 快照]→ 与 skill 同构、config-vs-DB 双轨消失 / 部门授权 = 两张 FK 表(`department_skill_rule`+`department_unit_rule`,各 FK→skill/tool_unit)[skill/unit 各一张、DB `ON DELETE CASCADE`、builtin 不入表、方向归行显式 effect 列、visibility 仅定未列部门默认、ABA 由构造消失] / 工具权限两轴[等级归工具定义 · 成员应用序=agent 宇宙 → dept 收窄宇宙 → skill-enable(dept 移除 key、skill 在收窄后宇宙内翻 disabled)] / **agent 宇宙 = `agent_unit` m2m[agent MD 种 seeded 行 + UI 挂 dynamic 行,解 DB-native 工具进宇宙]、skill 全 agent 可见[效果按 agent 宇宙收窄]、工具侧 dept 授权只到 unit 粒度[tool-set 与 MCP 对齐、无 member 例外 → 删 set-vs-member 冲突/override/检测整套机器;skill enable/agent MD 声明同样 unit 粒度,builtin=singleton unit]、MCP 只 server 粒度[不持久化动态成员]**),细节留各阶段开工敲定;**数据模型总览见专节**。
- **下一步**:**A 阶段(工具结果→富格式 artifact)** —— 引擎前置、独立可做、对沙盒数据入境有独立价值。但注意:skill MVP(文档 skill 跑在用户上传上)**不阻塞于 A**(上传已是 artifact、mount 已通);只有「数据工具→artifact→沙盒」类场景(DB→CSV→分析)才硬依赖 A。排序仍 A 先,因为它最小、最独立、且是后续富数据流入的脊柱第一环。
...
10. **部门授权 = 两张 FK 表 `department_skill_rule`(FK→skill)+ `department_unit_rule`(FK→tool_unit,含 tool/toolset/mcp);builtin for-everyone 不入表,agent MD 不动、引擎运行时组合。**
    - **两张表**:`department_skill_rule(department_id, skill_slug, effect∈{grant,deny})` FK→`skill`;`department_unit_rule(department_id, unit_name, effect)` FK→`tool_unit`。均 `ON DELETE CASCADE`,一资源多部门=多行。资源各带 `visibility`(external tool/set/mcp 随本轮 DB 化获得,见 Phase B/F + 原则 5);**unit 的 tool/toolset/mcp 细分在 `tool_unit.kind`,规则表不存类型列**。
    ...
    - 留待将来:参数级粒度(工具名+参数模式)、树 override 优先级。
...
| `department_skill_rule` | department ⟷ skill | 这部门**能用**哪些 skill(显式 `effect`) | admin UI |
| `department_unit_rule` | department ⟷ tool_unit | 这部门**能用**哪些 unit(显式 `effect`) | admin UI |
...
- **工具有效集**(三步、**dept 收窄宇宙本身、不是末端 AND 闸** —— 部门 `deny` 的工具直接从宇宙移除 key,skill 物理上够不到):
  ...
  - ② **dept 收窄宇宙** = 对 external 单元按 visibility 默认 + `department_unit_rule`(D 祖先链)显式 `effect` 解析,**部门未授权的直接移出宇宙**(key 删掉、连 enabled/disabled 状态都不再有;`public` 无规则行=授权**保留**、`department` 无 grant=**移除**,规则行只做 grant/deny 例外);**builtin 不过此步**(for-everyone、无 visibility、不进 dept 表,恒在宇宙)。
...
**包含**:
- **tool-set DB 模型 + 命名空间**:
  - external 工具/tool-set 本轮**从 config 迁入 DB + 前端 CRUD**(原则 5/决策 10:`config/tools/` 种子 = 不可变 seeded,UI 新建 = dynamic,无论来源都是 DB 行)。存储 = 「一平台多 endpoint + set 级描述/索引行 + `visibility` 列」。**凭证不进 DB**:只存「引用哪个 secret 名」,`{{TOOL_SECRET_*}}` 值留 env(见 Phase F)。
...
- **部门作用域 wire-in 的边界(消费全留 G)**:tool-set 将来接入 `department_unit_rule`(FK→tool_unit),方向 = 规则行显式 `effect`(tool-set `visibility` 只定未列部门默认,决策 10)。**B 只负责两件准备**:① tool-set DB 模型带 `visibility` 列(随本轮 DB 化,非 config frontmatter)、② 让 tool-set 有稳定 unit name 作 `department_unit_rule.unit_name` FK 目标。**部门规则的实际消费(查表 + 树解析 + 进 resolver)全在 G**(**两张规则表都在 C 建**——skill 表即消费、`department_unit_rule` 空跑到 G 才消费;dept 输入层 G 才加,见 line 101 分阶段输入);**B 的 resolver 骨架输入只有静态两样、不认部门规则**(修正:原写"resolver 减法认 grant"与 line 101 矛盾)。授权 UI 归 G。
...
- **server 配置(DB + 前端)**:MCP server 随本轮 DB 化 = **DB 实体 + 前端 CRUD**(原则 5/决策 10:operator 出厂 server = `config/mcp/` 启动种子进 DB、不可变;UI 新建 = DB 原生),带 `visibility` DB 列;per-deployment;无联网发现(Non-goal)。**凭证仍留 env**(`{{TOOL_SECRET_*}}`,见 line 160),DB 只存 server 定义。
- **部门作用域 wire-in(G 的消费者)**:MCP server 接入 `department_unit_rule`(FK→tool_unit;server 是一个 unit),方向 = 规则行显式 `effect`(server `visibility` 只定未列出部门的默认,决策 10;**server 的 `visibility` 是 DB 列**,随本轮 DB 化)。整 server 单元粒度(声明/减/加/enable 都在 server 粒度)= 决策 11 解 MCP 动态工具的同一抓手。消费(查表 + 树解析)在 G。
...
**包含**:
- **两张 dept rule 表 + 解析**(两表 + 解析地基都在 C 建,G 只加 unit 规则的消费):`department_skill_rule` / `department_unit_rule`(决策 10 已论证为何两张 FK 而非一张 polymorphic);判定 = 用户 `department_id` 走 `parent_id` 祖先链 ∩ 规则集(指父覆盖子树;命中后方向 = 行显式 `effect`,未命中走资源 `visibility` 默认),解析与已加载资源取交集(DB FK + cascade → 无陈旧规则)。两表共用同一棵祖先链解析(泛型 helper)。
...
- **管理 UI**:admin 按部门分配资源(grant 增删)+ 看每部门有效能力;前端复用 skill 管理页骨架(决策 1 的设置/管理页),按资源类型分 tab(skill / unit)。admin scope 守 `feedback-admin-scope-user-mgmt`(管共享资源/授权,不碰用户数据)。
- **规则体检 check 端点(admin 只读审计)**:扫两张 dept rule 表报**冗余/失效**(行 `effect` == 资源 `visibility` 默认方向 = visibility 漂移后的 dead row;两个被引用的资源表[skill / tool_unit]都有 DB visibility 列、一视同仁)。**孤儿不再可能**(DB FK + `ON DELETE CASCADE` 杜绝,无需检测)。**`effect` 显式列是前提**(方向派生时永远"自洽"、测不出漂移)。**只读非门禁**(正确性由 effect 列 + DB cascade 按构造保住),清理是独立 admin 动作。冗余判断绑定 v0 no-override 模型,将来启用「更具体规则胜」须同步改。

**到时再敲定**:**grant 删除** = DB `ON DELETE CASCADE`(两张 dept rule 表 + agent_unit/user_skill 均真 FK,删资源 DB 自动级联,无需 app-side cascade;原"config 类系统不碰 m2m + operator 运维契约"已作废);引擎"组合有效集"的缓存粒度(部门/用户级,资源变更少、可缓存);UI 是否支持"部门继承"可视化(指父覆盖子的展示);**参数级粒度**(工具名+参数模式、CC allow-rules 式,需调用时 arg 匹配)留将来(**unit 整体** grant/deny 已在 v0(`department_unit_rule` 整 unit 粒度),unit 之下的逐工具/参数粒度才是将来);**树规则 override 优先级**(深层更具体部门能否翻案祖先规则)—— v0 不做、祖先一刀覆盖整子树(决策 10"指父覆盖子树"),将来要"二级禁/某三级特批"再引入"最具体规则胜"。
...
- 2026-06-22 **修错锚点(用户点破:现无 skill 系统)**:原则 5「每 turn 一次小读可忽略」原拿「skill 今天即此」当先例,但 skill 未实现、锚点为空 → 改锚到真实存在的 per-turn DB 读(引擎今天每 turn 已重建 `MessageEvent` 历史),论证不变、依据落地。
- 2026-06-22 **原则 5 降级为不变式 + reconciler 串号修正**(用户:原则/决策两节定位该分清):原则 5 原是具体机制(reconciler 内部/entrypoint 槽/增量覆盖),名为「原则」实为「决策」→ 标题改回判断框(跨会话定义资源 = config 真相 + DB 缓存、共一条通路),机制下沉 Phase B。顺修一处既有串号:多处把 reconciler 引作「决策 5」(决策 5 实为 bundle mount)→ 正为「原则 5」,「决策 5/10」拆「原则 5/决策 10」。
- 2026-06-22 **`skill_md` 改存正文、剥 frontmatter**(用户:common practice L2 不渲 frontmatter):原写「SKILL.md 全文」会让 L2 重渲 frontmatter = name/description 与 L1 重复、`allowed-tools`/`license` 等系统元数据当噪声塞进上下文。改:`skill_md` 只存正文,frontmatter 字段已在消费列/`metadata`、原始结构在 blob → 零损失、导出仍无损;冗余从「三处」降为正文两处。
- 2026-06-22 **compatibility 校验从硬门槛移到软门槛 + 双门槛定调为 best-effort**(用户:硬门槛该只查 skill 本身、compat 是系统兼容性;且 compat 多半空着、确定性校验残废,缺则只能 agent 读内容判):硬门槛 = 只查 skill 本身良构(intrinsic);软门槛(verify agent)= 判系统兼容性,compat 有声明则校验、缺则从 body/scripts 推断。顺手:① 消除 compat-vs-环境 在两门重复;② 软门槛三查→两查(原 ③ asset 并入硬门槛链接解析);③ 显式写「两门皆 best-effort 过滤、非正确性闸,正确性兜底在运行时 `--network=none`+`--no-index` 响失败」总纲,防把过滤层当强一致门加码;④ 撤回误加的「硬门槛校验 compat 格式」(撞决策 9 缺失宽容)。净缩(去重 > 新增总纲)。
- 2026-06-22 **决策 11 区三处收口**(用户逐条审):① agent MD `tools:` **删 auto/confirm 等级字段**、迁移=一次性转写、旧字面量**不 lenient 当 enabled**而 loud-fail(防工具定义为 auto 时静默把 operator 的 `confirm` 降级、撞 P0);② **删「per-skill ACL」留待项**(P0 边界 + `confirm` 已兜住爆炸半径,不值 skill trust-tier 状态机);③ 澄清 **builtin 读源 = DB 的 `agent` 行**(agent 已物化进 DB)、「直读」指绕过 m2m join 而非读 MD 文件。
- 2026-06-22 **补多机部署留待项**(用户问多机 reconcile):advisory lock 只给互斥、非「全局只跑一次」,各机 bind-mount config 不一致 = 谁后跑赢 + 重启抖。记终态:config 成单一 artifact(烤镜像/同步同份)使 reconcile 幂等、或降为单点部署 job;**编排基线 = Ansible + docker**(沙盒 DooD + runsc 与纯 docker 宿主契合),**k8s 非推荐**(要把沙盒 spawner 重写到 k8s API + 每节点 runsc)。现单机 compose 不阻塞。
- 2026-06-23 **资源 identity 从 surrogate id 改 natural key(name/slug)**(用户:不想要隐藏于用户的 id 机制):skill/tool_unit/agent 本就不可重名 → 直接以 natural key 作 PK、m2m(agent_unit/dept/user_skill)全按 name 引用,与全系统按名寻址内外统一。**换来**:消掉隐藏 surrogate id + **ABA 由构造消失**(删名即 cascade grant、无 id 可复用)→ 删「不可复用 id 根治 ABA」留待项。**牺牲**:改名不再保留 m2m 规则(改名 = 删旧+建新、规则人工重授,reconciler loud-log 丢弃项);改内容(同名)仍 UPDATE 保规则。rename 罕见,换掉常驻 id 机器值。全文 `resource_id`→`resource_key`、`skill_id`→`skill_slug` 同步。
- 2026-06-23 **dept 授权 polymorphic 一张表 → 两张 FK 表**(reviewer P2/P3 + 用户点破物理目标只两类):unit-everywhere 后 dept 规则的物理目标只剩 `skill` / `tool_unit` 两类,polymorphic 一张表(无 FK、app-side cascade)的前提已塌。拆 `department_skill_rule`(FK→skill)+ `department_unit_rule`(FK→tool_unit)→ 均 DB `ON DELETE CASCADE`:**ABA 全程由构造消失**(reviewer P2 提的「孤儿规则 + 同 key 重建」链不再成立)、删整套 app-side cascade/孤儿处理、`resource_type` 列消失、check 端点只剩冗余检查。**P3 顺修**:ER 残留 `agent_id`/`unit_id`→`agent_name`/`unit_name`;identity bullet「m2m 全按软引用」纠为「全真 FK」。消费侧本就分型(skill 可见性 vs unit 宇宙收窄),两表反更顺。**P3 跟进**:钉死两张表都在 C 建(unit 表空跑到 G 才消费);check 端点 visibility 列指被引用资源表(skill/tool_unit)。
- 2026-06-23 **落定 search_tools/always_allow 两语义 + 清私有 memory 指针**(用户):① 压缩对工具发现归 compact_agent 提示词保留(同 artifact IDs)、schema 靠重 search;② always_allow key = 规范全名 `<unit>__<tool>`(同名跨 unit 独立、同工具跨 skill 共享)。顺手删全文私有 agent-memory 指针(`tool-ecosystem-positioning`/`permission-param-granularity-direction`,仓库外读者打不开),实质均已 inline → 文档自包含。
- 2026-06-23 **加 skill 侧 `EffectiveSkillSet` 单点可见性 resolver**(用户:工具有统一门槛、skill 会不会注入有闸 read 没闸):戳破 read_skill「镜像 read_artifact」的洞 —— read_artifact 只有 owner 一轴,照抄成 owner-only 404 会漏 skill 独有的 department 轴 → L1 按 user/dept 过滤、read_skill 却放行 dept-denied skill。解 = 立并列 resolver `EffectiveSkillSet(user)`,L1/read_skill/mount/REST 四面共用一条可见性解析;守 visibility=正确性(404、四面一致)、enabled=UX(仅 L1+前端)。形状比 `EffectiveToolset` 简单(无 agent 维度),与其只共 dept 祖先链 helper。避免的坑 = 每面各实现可见性致漂移、dept skill 被未授权用户 read。
- 2026-06-23 **EffectiveSkillSet 两处收口(reviewer P2/P3)**:**P2** = 「REST CRUD 四面全走 resolver」太宽 —— admin 管 public/部门授权时若被自身部门 `department_skill_rule` 过滤,会看不到别部门的 shared skill → 改「用户侧 read/list/mount 走 resolver;admin 管理端点按 admin scope + ownership/seeded、不按调用者部门 visibility 过滤」(守 `feedback-admin-scope-user-mgmt`)。**P3** = line 157 仍把 visibility/enabled 揉成一个「可见」,与新写的「visibility=正确性、enabled=UX」打架、会诱导把用户关掉的 skill 也 404 → 拆两句:`visible = owner/public/dept rule`(决定 read/mount)、`L1 enabled = visible ∩ (default_enabled + user_skill 覆盖)`(决定上不上索引);用户关掉的仍 visible、可 `/skill` 显式读。
- 2026-06-23 **substitution 整个关掉**(用户:纯 chat UI、不想加传参复杂度):原 v0 拟做 `$ARGUMENTS` 子集,但纯 chat 界面无命令行式「调用时带参」入口 —— 激活走 `read_skill`/`/skill`,参数即会话本身,`$ARGUMENTS` 无着力点;`` !`cmd` `` 还撞原则 8(激活零副作用)。定:substitution 全不做,正文占位符保留字面量(模型自解)+ 预装集人工去掉(原则 3 不静默 rewrite)。三处(Non-goals/原则 3/Phase D)同步。`${SKILL_DIR}` 是 mount 路径解析另一套、早已关掉,不在此列。
- 2026-06-23 **MCP 工具名 sanitize 简化为 check + loud-fail(用户 + web 调研,step-back design-creep)**:调研 MCP 官方 spec 工具名 pattern = `^[a-zA-Z0-9_-]{1,64}$`(snake_case >90%)= 我方 XML-tag-safe 字符集 → 合规 server 名字天然通过、零变换;原 sanitize 防的两类(冒号类非法字符 = Docker gateway 等本就违 spec 的罕见 bug;`__` 撞分隔 = 回解按已知 server 前缀剥离、本就不歧义)都不值一套有损变换机器。改:只校验字符集、违规 loud-fail 跳过该工具留其余可用。**作废 06-19 附带的 `(server,name)` injective / 可逆映射 / per-server 原名表 / 撞名 fail-fast 整套**——不做有损变换则「撞名」类不存在。代价 = 违规 server 那一个工具被丢(罕见 + 上游 spec bug,可接受)。
<!-- 新日志按日期顺序追加到此行上方 -->
