...
> 状态:规划完成;A 合 main,B 进行中(B-1/B-2/B-3 已合 main,B-4 后端已合 main,B-4 前端待开工)
...
- **下一步**:**B-4 前端(B 最后一片)** —— `export_openapi` + `npm run generate-types` 同步类型;管理页 = 工具 unit 列表/编辑(seeded 只读)+ agent_unit 挂载勾选 + 凭证写-only 配置;`visibility` 列展示但 G 才接 dept 授权 UI。后端 API 已就绪(`/api/v1/admin/tools/*`)。
