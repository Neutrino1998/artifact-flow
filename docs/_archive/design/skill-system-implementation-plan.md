...
5. **种子机制统一:skill/tool/agent 三类共用一个 `config-seed→DB reconciler`,全部 DB 物化(用户拍 B)。** 三者都非 artifact(artifact session-scoped;它们 user/系统-scoped 跨会话),preinstalled bundle 一份共享。
   - **三态**:`seeded`(config 种子、reconciler 拥有、UI 不可改;**config 即唯一作者真相、DB 是物化缓存、git 可版本化**)/ `dynamic`(UI 新建、DB 原生可变,仅 skill/tool;agent 暂 seed-only)。
...
| 表 | 连接 | 回答 | 谁写 |
|---|---|---|---|
| `agent_unit` | agent ⟷ tool_unit | 这 agent **暴露**哪些 external 单元(= 宇宙) | `seeded`=agent MD 经 reconciler 种;`dynamic`=UI 勾选挂载 |
