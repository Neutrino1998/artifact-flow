...
**到时再敲定**:frontmatter 精确字段子集(v0 = `name`/`description`/`allowed-tools`?);skill 与 B 的 tool-set 是否联动(**开放分叉**:CC/opencode 完全分离;我方**可选**让 skill 的 `allowed-tools` 调用时 auto-`search_tools` 预载其 set —— v0 倾向保持分离、再议);用户侧激活的正文注入通道(合成 `read_skill` tool_result〔可随压缩淡出、倾向〕vs 常驻 `<activated_skills>` 块,见进展决策③,C-3 敲定)。

**进展**:规划细化完成(2026-06-30,3 决策见 changelog);**C-1 存储侧已合 main(`aef812d`);C-2 引擎消费实现完成**(待提交)。
