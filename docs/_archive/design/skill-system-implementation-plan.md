...
- 2026-06-23 **reconciler 第二条改 visibility 路径补清规则 + 冗余表述修正(reviewer P1/P3)**:**P1**(真漏)= 「改 visibility 清规则」原只落 Manager UI 路径,但 reconciler 从 config UPDATE seeded 资源时(line 192「同名 UPDATE 保 m2m」)若 `visibility` 列变更,旧例外熬过更新、方向反转 = 把本轮消的 bug 带回 → 明确 reconciler 检测 `visibility` 变更也同事务清 dept 规则,**两路都清**。**P3** = 「冗余结构上不可表达」不全对:同向默认冗余确不可表达,但祖先/子树冗余(父已例外、子再建)仍可表达 → 改「非正确性问题(解析一致),UI 拦截或接受、不上审计」;结论(无 check 端点)不变。
<!-- 新日志按日期顺序追加到此行上方 -->
