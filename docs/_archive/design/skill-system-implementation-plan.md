...
| C | Skill 核心(存储 + L1 注入 + `read_skill` + 权限/上下文覆盖 + 部门授权解析地基) | **规划细化**(拆 3 片 + 3 决策,见该节进展 + changelog 06-30),未开工 |
...
**到时再敲定**:frontmatter 精确字段子集(v0 = `name`/`description`/`allowed-tools`?);skill 与 B 的 tool-set 是否联动(**开放分叉**:CC/opencode 完全分离;我方**可选**让 skill 的 `allowed-tools` 调用时 auto-`search_tools` 预载其 set —— v0 倾向保持分离、再议)。~~用户侧激活的正文注入通道~~ **已敲定(C-3)= 注入 `USER_INPUT` 正文**(同 force_compact/上传路径,非合成 tool_result):能力(grants)已由 controller seed `active_skills` 单独烤开,合成 tool_result 唯一剩的活只是搬正文 → 直接复用现成 USER_INPUT 注入块最省,不伪造 tool 事件。

**进展**:规划细化完成(2026-06-30,3 决策见 changelog);**C-1(`aef812d`)/ C-2(`e590946`+`0c5e38d`)/ C-3 backend(`82094c6`)已合 main;C-3 前端实现完成**(侧栏技能管理页 + composer 激活选择器,待提交)。**C 阶段全部完成。**

- **C-3 backend 落地(激活路径 + 用户侧管理 REST,镜像 B-4 三层)**:**① 激活路径** = `POST /chat` 加 `activate_skills:[slug]`(形状同 `force_compact`;空文本+激活 = 放行)→ controller 按 `EffectiveSkillSet.visible` 校验(可见=正确性,**不要求 enabled** —— 显式激活自己关掉的可见 skill 是合法 opt-in;不可见静默丢、不 404 免泄露)、与 `parent_active_skills` 去重取**本轮新激活**、对全 agent toolset `activate_skill` 烤能力、短 session 取 `skill_md` 正文传 engine;engine `create_initial_state(activated_skill_bodies=)` → `execute_loop` 把正文注入 **USER_INPUT 正文**(同 force_compact/上传块,仅 LLM 可见、随历史带下 → 只注新激活的,往轮的早在其当轮 USER_INPUT)。**② 管理 REST**(纯用户侧,**无 admin 页** —— seeded skill 归 config 只读、C-3 前无动态 skill 可管)= `SkillManager`(三层 Manager,复用 `EffectiveSkillSet` 单点可见性闸,序列化 `{enabled 有效态, default_enabled, is_overridden}`)+ `SkillRepository.set_user_override`(upsert `user_skill` 稀疏覆盖、stage-only)+ `GET /api/v1/skills`(列可见 + 有效态)/ `PUT /api/v1/skills/{slug}/enabled`(个人 toggle,不可见→404)。**个人 toggle 控 `enabled`(UX:进不进 L1)非 `visible`(正确性:系统定,用户改不了)**。测试:engine 注入 E2E(带文本/纯激活)、controller 空输入闸放行、`SkillManager` DB(list/toggle/404)、`test_skills` API(GET/PUT/401/404)。`export_openapi`→`generate-types` 已跑。**③ 前端**(全用户,非 admin)= 左侧栏新增「技能管理」栏(与 文件面板/搜索对话/新建对话 并列,`activeMode='skills'` 中间面板接管,同 conversationBrowser)→ `SkillManagementPanel` 列可见 skill + 个人 enable 开关(乐观写 `PUT`);对话内 composer 加「激活技能」弹层选择器(compact 按钮旁),armed 显 chip、随下条消息发 `activate_skills`、成功清、放宽空输入闸。**两个前端 UX 决策**:(a) **picker 只列 `enabled` skill**(禁用的连激活选择器都不显 —— `enabled` 统管 L1 索引 + picker;后端激活闸仍是 `visible`/permissive,过滤纯前端 UX,每次开 picker 重取避陈旧);(b) 管理入口在侧栏而非 UserMenu。tsc/lint/vitest 全绿(202)。**动态 skill 创建/编辑 + admin 页 + dept 授权 UI 留 E/G。C-3 完成。**

- **C-2 落地(引擎消费 skill,镜像 B-2/B-3)**:**① 可见性 resolver** `EffectiveSkillSet`(`src/core/effective_skillset.py`,并列 `EffectiveToolset` 非合并)= visibility(private owner / public 默认可见·dept 例外=deny / department 默认隐·dept 例外=grant,方向派生 visibility)+ enabled(default_enabled + user_skill 覆盖);只共用部门祖先链 helper `src/core/department_resolver.load_ancestor_ids`(父覆盖子树,G 复用)。**② skill 能力轴** = `EffectiveToolset` 加 `skill_grants` 字段 + `activate_skill()`,`resolve_effective_toolset(skill_snapshot=)` 预烤 `{slug:{full_name:level}}`(skill allowed-tools ∩ 本 agent disabled 池,经共享 `resolve_allowed_tool_entry`);引擎 read_skill 后 append `active_skills` + 对**所有** agent toolset `activate_skill`(纯字典 merge、本回合即生效,无闭包)。**③ L1** = ContextManager `_build_available_skills` 注入 `<available_skills>`(尾部 reminder、同 catalog,避 toggle 打 APC)。**④ L2** = `read_skill(slug)` 请求级工具(`src/tools/builtin/read_skill.py`,镜像 read_artifact 契约 `inf`/`AUTO`,可见性走 EffectiveSkillSet 非 owner-only;声明式回填 `metadata.activated_skill`);`SkillService` lazy 取正文(B-5 短 session)。**⑤ 状态** = `active_skills` 照抄 `always_allowed_tools`(父消息 metadata 进出 + create_initial_state + 回合末持久化)。**⑥ 装配** = `controller_factory` 收 `user_id`、一条短 session 读 skill 快照 + user 覆盖 + 部门命中(dept_id 从 DB 取不信 JWT)、`resolve_all(skill_snapshot=)`、建 read_skill 并在有可见 skill 时注入每 agent toolset。测试:`test_effective_skillset` / `test_skill_grants` / `test_read_skill` / `test_skill_resolution_db`(DB 祖先链)/ engine 激活 E2E / ContextManager L1,加既有全绿。**reviewer 一轮收口**:`skill_grants` 改从**可见子集**烤(非全量 snapshot)—— 跨回合恢复 `active_skills` 时被撤销(admin 撤 dept/`public→department`/换部门)的 skill 授予下轮消失,by-construction(非恢复循环加闸);能力跟随可见性、名单仍 sticky(visible=correctness / enabled=UX 落到工具授予轴)。另两条 reviewer findings(`Skill.source` 缺 server_default = 与最近兄弟 `ToolUnit.source` 一致、无触发,leave;`<available_skills>` slug 转义 = C 阶段 operator 控无触发,正解归 C-3 slug 格式校验)按 step-back#1 不做。**C-3 待开工(API + 前端管理页)**。

- **C-1 落地(纯加法、引擎零消费)**:4 表 `skills`/`user_skills`/`department_skill_rules`/`department_unit_rules`(natural-key PK、m2m 真 FK + CASCADE,plural 表名随 codebase)**就地写进 squash `0001`**(沿 has_blob/B-1 姿态,无 0002);`skill` 6 字段按消费分流(消费列 + `meta` JSON〔属性名避 SQLAlchemy 保留的 `metadata`、DB 列名仍 `metadata`〕 + `skill_md` 正文 + `bundle` blob〔C 单 SKILL.md 恒 NULL,bundle skill 归 D〕)。reconciler 加 `parse_skill_seeds`/`_reconcile_skills`(幂等 upsert/prune/clear-on-visibility,同 tool/agent 范式)+ `load_skill_snapshot`(轻量 user-agnostic、不载 skill_md/bundle);**`allowed-tools` import 校验走共享 `resolve_allowed_tool_entry`**(`tools/base.py`,import+runtime 同一函数,reviewer P2)、解析不到 = warn 不 fail(决策 11 line 237);seeded skill 拒 `private`。**两个 dept clear/prune 钩子接通**:`department_unit_rule` 表此刻存在 → `_clear_dept_rules_for_unit` no-op 改真删 + `_prune_unit` 一并删(B-1 留的 TODO 收口);skill 侧对称。种子 = `config/skills/precise-artifact-edits/`(单 SKILL.md 示例,真正策展集留 E)。测试 `tests/reconcile/test_skills_reconcile.py` 11 passed,db/reconcile/repos/core 417 passed。**C-2 待开工**。

- **增量拆分(遵 B 逐片合 main)**:**C-1 存储侧**(纯加法、引擎零消费,镜像 B-1)= 4 表[`skill`/`user_skill`/`department_skill_rule`/`department_unit_rule`]就地写进 squash `0001` + `parse_skill_seeds`/`_reconcile_skills`(幂等 upsert / prune / clear-on-visibility)+ `load_skill_snapshot`(user-agnostic 定义)+ **接通两个 clear-on-visibility 钩子**(现有 `_clear_dept_rules_for_unit` 的 no-op 此刻有 `department_unit_rule` 表可删)+ 一个最小无 bundle 种子 skill(端到端 smoke,真正策展集留 E)。**C-2 引擎消费**(镜像 B-2/B-3)= 部门祖先链 helper + `EffectiveSkillSet` resolver + `<available_skills>` L1 + `read_skill` 工具 + `active_skills` 接进 `EffectiveToolset` + 状态生命周期照抄 `always_allowed_tools`(父消息 metadata 进出)。**C-3 管理 API + 前端**(镜像 B-4 前端 + 三层 Repo/Manager/Router)= CRUD + 用户 opt-in toggle + 管理页 + `export_openapi`→`generate-types`;dept grant 授权 UI 留 G(`visibility` 列展示不接授权)。
...
> 规则:一条 = **一句里程碑决策 + 为何不选替代**(目标 ≤ ~400 字)。逐轮 reviewer 收口压成一句「lasting lesson」;措辞/一致性/cosmetic/UI 小修**不单列**,并入当阶段「收尾」条。commit hash 串、文件清单、逐 finding、被取代的中间态留 **git history**(每个 commit ≈ 一条细节日志)。
...
- 2026-06-24 **A 阶段落地(首个实现)**:`ToolResult.artifact`(声明式)+ `ingest_tool_result` 复用 `create_from_upload` 内核;web_fetch 文件旁路。三条 reviewer 驱动决策:**① artifact = 单一表示(content XOR blob)** —— 删「blob 原件 + 转换文本」双表示,「是否二进制」从可走私的 metadata 升级为 `Artifact.has_blob` 列(by-construction 灭 text 伪装 binary 走私)。**② 落盘统一 loud-fail** —— 必须落盘却失败一律响失败,删 fail-open(退回超长原文正是落盘要防的)。**③ model-facing XML ≠ parsed XML**(已记 CLAUDE.md)。`has_blob` 就地写进 squash 的 `0001`。
- 2026-06-25 **B 拆 4 片增量合 main + B-1 落地**:B 切成 B-1(DB 模型+reconciler+snapshot 读侧)→ B-2(resolver+引擎切快照+decision-11)→ B-3(deferred+search_tools)→ B-4(provider 缝+前端)。**关键边界决策 = decision-11 的 agent MD 重写从 B-1 推到 B-2**:B-1 若改 MD 格式,仍读旧格式的引擎被迫一起翻 → 塌掉增量边界。故 B-1 = write-only 物化(引擎不动、纯加法),B-2 才把物化接进引擎 + 同步 MD 重写。
...
- 2026-06-26 **B-3 落地(渐进式披露,纯 prompt 级)**:`generate_tool_instruction` 拆成 `generate_tool_grammar`(留 system 前缀保 APC)+ `render_tool_docs`(catalog 挪 `<available_tools>` reminder);`tool_unit.defer` 只渲索引行,`search_tools` 按需补 schema。**catalog 留 reminder 是有意**(reviewer 驳「挪回前缀」):C 阶段 skill 能 enable agent-disabled 工具 → 可调集易变,放前缀则 toggle 打掉整条历史 APC,放尾部把易变性隔离在本就不缓存的末条。
- 2026-06-26 **B-4 后端落地(凭证加密 + CRUD)**:**① 凭证统一加密落库** —— `tool_credential` + Fernet 单主密钥 + `CredentialResolver` lazy 到 execute、只解被调 unit、密文不进快照/catalog;resolve 一条路 = 读库解密(seeded 由 reconciler 从 env 加密、判变靠解密旧↔比 env 新支持轮换)。红线:只发受信 backend、沙盒不拿、per-user 透传 defer。**② 后端 CRUD**(三层 Repo/Manager/router):external unit dynamic 增删改(seeded 只读)、agent 挂载、凭证写-only(GET 永不回明文);撞名 by-construction 主闸落写入期。**③ provider 缝不新建抽象**(MCP 未到、避过度防御):既有 `provider` 列 + snapshot 分派已够,F 加 mcp 分支纯加法。
- 2026-06-29 **B-4 reviewer 两轮收口 + 乙2 + 拆出 B-5/B-6(7 commit)**:reviewer 两轮(15 findings + 复审)。关键落地:主密钥强制 fail-to-start(`cc5ca36`);env 缺失保留旧密文不删(env-absent 是模糊信号,不在其上销毁机群共享态)。**关键 = 用户 step-back 暴露更深架构**:credential lazy 读骑 turn-long session 与 artifact 读同型(连接整轮被占 + 无 retry)→ 本质 = turn-long session 不该存在 → **拆出 B-5「退役 turn-long session」**。**乙2 = reconcile 单点门禁**(release/serve 拆分 + nginx 变量 proxy_pass);**CVE 拆出 B-6**。**过度收口教训**:#7「repo 漏删凭证」我做了「自洽 repo」修 → 引入分层渗透 + 死代码 → 回退,因为那个孤儿化无实际触发者 → 沉淀为**全局 CLAUDE.md step-back 第 1 信号「无可达触发的缺陷别 fix」**。
- 2026-06-29 **B-5 退役 turn-long session**:勘察发现迁移已 ~70% 完成,`controller_factory` 预开的 turn-long session 只剩两消费者 = 引擎期 **artifact 读** + **snapshot 读**。落地:snapshot/artifact 读各开 `db_manager.with_retry` 短 session(全物化成 dataclass/detached、ORM 不逃逸;WorkingSet 留实例做 turn-live 缓存;REST-only `get_version` 故意留 bound repo 避免改短 session 反造逃逸);**credential 退回 lazy decrypt-at-execute**(`CredentialResolver` 持 db_manager、execute 期开短 session 只解被调 unit、**零明文缓存**,消 N1 明文驻留整轮);controller setup + 事件持久化 guard 改走短 session,turn-long `async with` 整段删。**用户决策(AskUserQuestion)**:snapshot 改短 session 已免费解决凭证 session-正确性,退回 lazy 唯一额外收益是 N1(在加密落库威胁模型外),但合规/可审计姿态(解密值不驻留超过其调用)优先 → 选退回 lazy。
- 2026-06-30 **B-5 reviewer 一轮收口(`b78558e`)= lasting lesson**:B-5 把 setup 写包进 `_with_db_retry` 却没满足 `with_retry` 的隐含契约「**fn 必须幂等**」(失败从头重跑)。两处不幂等修复:`add_message_async`(瞬断重跑撞重 → 遇 `DuplicateError` 当成功)+ `start_conversation` 的 `conv-{uuid}`(提到被重试 lambda 外生成、避免孤儿会话)。**沉淀**:凡 `with_retry` 包裹的写必须幂等 = 稳定幂等键在边界外定 + 遇「已存在」当成功;契约写进 docstring + `TestRetryIdempotency`。
- 2026-06-30 **B-6 依赖 CVE 升级**:`pip-audit` 抓到 3 包 6 漏洞(pydantic-settings / python-multipart / starlette —— 后者是 fastapi 传递依赖,加直接安全 floor `starlette>=1.3.1`),按 DEP-02 在 `python:3.11-slim` 内 `pip-compile` 重生成 lock、re-audit clean。
- 2026-06-30 **B 收尾验收(dev-Mac docker)**:工具生态 + 部署门禁全链路实测通过 —— config-seeded 与 UI-dynamic 两条建→快照→注入→调用→回流都验;defer/非 defer 注入形态对照;confirm 授权回流;compose release/serve 门禁(advisory lock 下 alembic+reconcile 一次、backend `service_completed_successfully` 闸住、重跑幂等)。**多副本 `--scale backend=2` 实证 + 纠误**:两副本共享 Redis RuntimeStore;**Docker 网桥 DNS 原生轮询**(中立容器 10 发 5/5)—— nginx 不是轮询的必要条件,它的作用是宿主入口 + **防 DNS pinning**(静态 `upstream`/keep-alive 只解析一次会钉死副本 → 变量 `proxy_pass`+`resolver` 强制每请求重解析)。撞名 fail-closed 实证(seeded 撞 dynamic 同名 → reconcile loud-fail、写前阻断)。**留真机**:真 nginx LB 入口(caddy 要 ACME)+ 跨副本执行续接(A 发起 / B 经共享 Redis resolve interrupt)。
- 2026-06-30 **C 规划细化(拆 3 片 + 3 决策,未开工)**:**① 增量 = C-1 存储/reconciler(写侧物化、引擎零消费)→ C-2 引擎消费(resolver + L1 + read_skill + active_skills)→ C-3 API/前端**,逐片合 main(同 B,纯加法无破坏中间态)。**② skill 能力轴落地简化 = `EffectiveToolset` 烤 `skill_grants`(非引擎重解析闭包)**:用户点破不变量 —— skill 只能 enable agent-MD-`disabled` 的工具,而 disabled 工具本就在宇宙快照里 → resolver 解析时顺手按 slug 烤 `{slug: {full_name: level}}`(skill `allowed-tools` ∩ agent disabled 池、等级取自工具定义),激活 = 引擎一句 `permissions.update(skill_grants[slug])` 纯字典 merge、本回合即生效,引擎不持 snapshot/闭包。修正初版「传重解析闭包进引擎」。**③ 用户侧激活 = 按钮 + 特殊输入路径(非 `/命令`)**:`/命令` 对一般用户不友好 → UI 按钮选 skill,下条 `POST /chat` 带 `activate_skills:[slugs]`(同手动 compaction force-flag 形状),controller 经 `EffectiveSkillSet` 校验可见性后 seed `state["active_skills"]`;正文注入通道(合成 `read_skill` tool_result vs 常驻块)留 C-3,C-2 先只落模型驱动 `read_skill`。
- 2026-06-30 **B 收尾期收口一批**:(1) **`response_extract` 统一到真 JMESPath** —— 旧手写点号解析器(只认 `$.a.b`)与 UI/注释三处名实不符、且我们自己 ship 的 `$.results[*].id` 它静默空;换 `jmespath` 库(单一权威 spec),两写入边界(seeds/CRUD)`compile` 校验语法 loud-fail、匹配不到显式回 "matched nothing" 不再静默空;reviewer 报「无 `$.` 兼容」按 step-back#1 驳回(B 未发版→零存量,clean break 优于永久双语法)。(2) **`show_example` per-tool → 部署级 `config.RENDER_TOOL_EXAMPLES`** —— 示例需不需要取决于部署用什么模型、与具体工具无关 → 粒度该是 deployment 非 per-tool(整条退役 per-tool 列/UI/schema,无老数据迁移直接删 `0001` 列);前置 = grammar 硬化(调用结构写进恒稳 system 前缀),examples-off 真模型(最小 35B + deepseek)实测仍产出合法 tool_call。(3) **internal agent 禁挂载工具** —— compact_agent 等不跑工具循环,mount() 拒绝 + 列表过滤(后果实为双重惰性:compaction 绕开工具注入,故是 footgun 防呆非 correctness)。(4) cosmetic 一批:`release: not found` stderr 噪音(shell 双引号内反引号命令替换)、参数编辑器 default/enum 布局、工具管理图标换标准扳手、删除 unit 弹窗去勾选对齐权限弹窗。**craft lesson**:补测时 Edit 锚点未覆盖方法尾部 3 行泄漏断言 → 新方法插进去把守卫挤进惰性测试静默失效、suite 仍绿(`d0b0ef8`);Edit old_string 要确认到方法真正结尾。
