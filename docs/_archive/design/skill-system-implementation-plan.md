...
- **当前**:**A 阶段已实现并合 main(见变更日志 06-24);B 起待开工。** 沙盒主线(底座)已收官。架构主分叉已收口(披露归工具层 / 可见性=visibility+default_enabled+稀疏覆盖 / 依赖三层离线 / 导入硬软双门 / read+mount 复用零新机制 / external 工具/tool-set/mcp + agent 全 DB 物化[通用 config-seed→DB reconciler、skill/tool/agent 一套种子机制、config 仅作启动种子、注册表每 turn 一次 DB 快照]→ 与 skill 同构、config-vs-DB 双轨消失 / 部门授权 = 两张 FK 表(`department_skill_rule`+`department_unit_rule`,各 FK→skill/tool_unit)[skill/unit 各一张、DB `ON DELETE CASCADE`、builtin 不入表、方向派生自资源 visibility(行=例外、无 effect 列)、改 visibility 清规则、ABA 由构造消失] / 工具权限两轴[等级归工具定义 · 成员应用序=agent 宇宙 → dept 收窄宇宙 → skill-enable(dept 移除 key、skill 在收窄后宇宙内翻 disabled)] / **agent 宇宙 = `agent_unit` m2m[agent MD 种 seeded 行 + UI 挂 dynamic 行,解 DB-native 工具进宇宙]、skill 全 agent 可见[效果按 agent 宇宙收窄]、工具侧 dept 授权只到 unit 粒度[tool-set 与 MCP 对齐、无 member 例外 → 删 set-vs-member 冲突/override/检测整套机器;skill enable/agent MD 声明同样 unit 粒度,builtin=singleton unit]、MCP 只 server 粒度[不持久化动态成员]**),细节留各阶段开工敲定;**数据模型总览见专节**。
- **下一步**:**B 阶段(工具渐进式披露:tool-set DB 模型 + `search_tools` + 通用 `config-seed→DB reconciler` 底座 + `EffectiveToolset` resolver 骨架;MCP 适配缝)** —— A 已交付「backend tool → artifact」富数据入境第一环;B 是 C(skill 复用 reconciler)/F(MCP = 又一 provider)的共同地基。
...
| 阶段 | 内容 | 状态 |
|---|---|---|
| A | 工具结果→富格式 artifact(`create_from_upload` 的第三调用方) | **已完成**(合 main,见日志 06-24) |
