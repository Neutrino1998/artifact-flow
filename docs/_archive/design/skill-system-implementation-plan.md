...
10. **部门授权 = 两张 FK 表 `department_skill_rule`(FK→skill)+ `department_unit_rule`(FK→tool_unit,含 tool/toolset/mcp);builtin for-everyone 不入表,agent MD 不动、引擎运行时组合。**
    ...
    - **改 visibility = app-side 清该资源 dept 规则行**(同事务、定向、**留 `user_skill`**):visibility 是 UPDATE 非 DELETE → DB `ON DELETE CASCADE` 不触发;资源级 cascade 又会误伤 user_skill/agent_unit → 不用 trigger,在 visibility-update 的 Manager 方法里定向删 dept 规则;UI loud-warn「将清除 N 条例外」。换得:表无冗余 by-construction、无 check 端点、admin 改后在新 regime 重授。
...
- **无规则体检 check 端点**(原冗余/孤儿审计整条删除):冗余行**结构上不可表达**(无 `effect` 列、行=例外、跟默认同向=不建行)、孤儿由 DB FK `ON DELETE CASCADE` 杜绝、visibility 漂移由「改 visibility 清规则」(决策 10)按构造防住 → 无 dead row 可查。正确性由「派生方向 + clear-on-visibility + DB cascade」三者按构造保住,不需运行时审计。
...
- 2026-06-23 **修 `__` 命名规则两处与新 F 段矛盾(reviewer P2)**:line 109/186 旧写「`__` 恰好一次、名只用单 `_`」对 tool 段也成立,但 F 已放开 MCP tool 名可含 `__`(spec 合法)、解析按已知 server 前缀剥离 → 旧规则会让实现者错拒合法 MCP 名。统一分层:**unit 名禁 `__`**(operator 控,保 `<unit>__` 前缀与 `<unit>__*` 通配无歧义)、**tool 段可含 `__`**、解析一律按已知 unit 前缀剥离不 split;静态作者成员名仍建议单 `_`(简化、非强制)。
- 2026-06-23 **dept 规则删 `effect` 列、方向改派生 + 改 visibility 清规则(用户,安全反转 311)**:前情 = 已定「改 visibility = 清该资源 dept 规则」。311 当初存显式 effect 的**唯一**理由(改 visibility 会让派生方向静默翻转既有行、反授权)**因清规则失去载体**——行不再熬过 visibility 变更。故退化:**删 `effect` 列**,一行 = 部门「例外」、方向 = 资源 `visibility` 默认反向(public→deny、department→grant);冗余**结构上不可表达**(同向=不建行)。**连削三样**:写入拒冗余(不存在)、check 体检端点(无 dead row 可查)、「显式 effect 抗漂移」整套论证。清规则 = visibility-update 的 Manager 方法里 app-side 定向删(UPDATE 非 DELETE、DB cascade 不触发;只删 dept 规则、留 user_skill;不用 trigger 因资源级 cascade 误伤 user_skill + 双方言 + 隐藏逻辑)、UI loud-warn。**未来代价**:启用「最具体规则胜」override(嵌套反向例外)时需把 per-row 方向/effect 列加回。
<!-- 新日志按日期顺序追加到此行上方 -->
