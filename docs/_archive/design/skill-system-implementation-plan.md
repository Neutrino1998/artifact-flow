...
- **L2 = `read_skill(slug)` 工具**(决策 4):独立工具、镜像 read_artifact 契约(`inf` 不二次 persist、`AUTO`),**死简单——返回 `skill_md`(正文、去 frontmatter)、无 path 参数、后端不解 zip**;输出附「其余细节(含 references)须 mount 进沙盒读」提示;**不合并进 read_artifact**(身份空间/store/Manager 不同)。用户 `/skill` = 等价入口(同走 read_skill 取正文注入,多一个 UI 触发)。激活无副作用、非状态切换(原则 8)。
