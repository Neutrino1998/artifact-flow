...
- 2026-06-12 **D 段开工——本机准备完成**:三决策(feat/sandbox **先不合 main**、backend 走**一次性测试包**不动 intranet 分支、本次 trip **仅 arm64** 冻结——x86 等有 ≥4.14 内核的 runsc 盒子再补)。**发现并补部署缺口:容器化 backend 的 DooD 接线整体缺失**——新增 `deploy/docker-compose.sandbox.yml` opt-in overlay(sock=宿主 root、无沙盒部署不挂;scratch 根**宿主↔容器同路径** bind,daemon 按宿主路径解析 bind source,`ARTIFACTFLOW_SANDBOX_SCRATCH_ROOT` 单变量喂 compose 两侧+应用)+ deployment.md「沙盒执行环境」节(gVisor/镜像/loop 池子 host-prep 命令入册)+ `.env.intranet.example` 沙盒段(RUNTIME 生产禁空)。介质 20260612 三件套出包:沙盒镜像 arm64(**含 git**,manifest git 2.47.3)、verify tar(含 git+dubious-ownership 新探针)、app+deploy+config+infra dsmoke 包(全 arm64);gVisor 复用 §B 的 aarch64 包。**本机 arm64 compose 栈(真容器化拓扑)DooD 冒烟 5/5 全绿**(`tests/manual/dood_compose_smoke.py`:sock 通路/exec/uid1000/路径同一律/拆除)。真机清单:PAGE_SIZE+unshare 预检 → gVisor install+smoke → run-all.sh **重冻结 arm64 image id** → loop 池子 host-prep+配额探针 → agent 流端到端(CONFIRM/Word 场景/cancel-kill/uid 属主双向)→(可后置)XFS project quota 评估。
- 2026-06-12 **沙盒状态动态注入落地**(用户点出痛点:模型不知道每轮要重新 mount——历史里上一轮 mount/bash 记录是"文件还在"的伪证,工具描述的静态 ephemeral 规则压不过历史证据):ContextManager 动态 `<system-reminder>` 增 `<sandbox_status>` 三态(not_started=明示工作区空/旧 mount 失效,**对冲伪证的主力态**;running=工作区第一层清单,`SANDBOX_STATUS_MAX_ENTRIES`=20 条数帽+显式 (+N more),顺带给 persist path 决策省一次 ls;unavailable=sticky 原因复述,省模型再撞一次)。接线=`SandboxSession.status_snapshot()`(同步轻量;枚举走 `sandbox_fs.list_dir` fd 钉住纪律,文件名控制字符→� 防伪造清单行)→ execute_loop 新 duck-typed `sandbox_session` 参数(to_thread 取快照,失败只缺席不阻断)→ controller/factory 传递。门控=agent 授沙盒工具 AND 快照非 None(compact 等永不见;旧调用方零破坏)。**提示分层补齐第四层:能力→描述、契约→inventory 文案、状态→动态注入、场景→skill**。后端 1259 全过(+13)。镜像无涉,不重打包。
