...
- **2026-06-10 C-reap review 收口**(两条;明确 InMemory 部署边界 + 停机顺序):
  ...
  - **[P2 已修] 停机时 reaper 先于 runner shutdown 停,兜不住 shutdown 期 close() 失败的孤儿**。原顺序在 `close_globals()`(内含 `runner.shutdown()`)前就停了 reaper,而 `SandboxSession.close()` 正是在 runner shutdown 时跑;单副本停机后若某 close 超时/失败,无下一个 startup scan 收尾 → 孤儿跑到下次启动。改顺序:**runner 先 shutdown(提前显式调,close_globals 内二次调 `_tasks` 已空 no-op)→ reaper `final_sweep()` 最后一扫 → 再 stop → close_globals**。`final_sweep` 的 grace 按 store 分:进程本地(此刻确无在途 turn)忽略 grace 全收,共享(兄弟可能正起新 turn)保留 grace 不误删。后端 1211 全过(+6);真机矩阵 12/12;`test_sandbox_reaper.py` 增 gate(4)+ final_sweep grace 二态(2)。
