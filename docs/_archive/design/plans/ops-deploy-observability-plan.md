...
- **当前**:Phase D 代码已落仓(2026-07-04)——`fleet.sh`(preflight/deploy/status/rollback)+ `fleet.conf`(逐行 role/host/arch/scale 表)+ FLEET.md runbook。单机形态(host=local)Mac 可测端到端、已验(help/preflight/status/dry-run/校验/rollback 空 state 报错全绿);多机 cross-host 已成文但 gated off,标真机验收。Phase A/B/C 此前已完成/落仓(B/C 各留一笔真机验收挂账)。
- **下一步**:三笔真机验收整批随那条大功能线上线的发版窗口(D 是为它的部署便利而建):(a) D 两台真机 `preflight`+`deploy` + un-gate cross-host 三缝(backend 端口发布 / 静态 upstream / per-host env DB URL);(b) C autoheal systemd 周期闭环;(c) B 内网 nginx→Caddy 切换。B+C+D 首次同窗上真机的大爆炸集成风险已知(窗口不近、D 不碰业务代码,攒批可接受)。
...
| D | 多机 fleet(fleet.conf + fleet.sh 单命令发布 + LB 模板生成 + env 单源推送) | **代码已落仓**(2026-07-04);单机形态 Mac 验;多机 cross-host gated off,标真机验收 |
...
- **2026-07-04 Phase D 代码落仓(多机 fleet 统一发布入口)**。三个文件:`deploy/fleet.conf(.example)`(逐行 `role host [arch=] [scale=]` 表——bash while-read 解析,无 section 状态机、无 source 执行风险,多 app 行天然表达;基数校验 = 恰一 release + 一 lb + ≥1 app)+ `deploy/scripts/fleet.sh`(`preflight`/`deploy [--dry-run]`/`status`/`rollback` 四子命令,`set -uo pipefail` + 显式 die 驱动)+ `deploy/FLEET.md`(自洽 runbook)。**三个到时再敲定项定案**(用户「按推荐」):① conf 格式 = 逐行 key=value 表;② infra = 专机 compose(`--profile infra`,复用 `--with-infra` infra tar);③ 控制机 = 内网跳板机(Mac 构建→tar 带入→跳板 ssh 两台;单机退化=就在那台跑 host=local 免 ssh)。**单机=多机退化的构造**:传输层 `run_on`/`copy_to` 对 `local` 走本地、其余走 ssh,同一段 deploy 序列——单机走 compose 原生 `depends_on`(release gate + 健康序)编排,多机由 fleet.sh 自己接管跨机顺序(compose depends_on 是同机的)。**arch 是校验非选 tar**:release.sh 每次单架构、文件名不带 arch(只 manifest `Platform:`),fleet.sh 读 manifest 版本/平台 + 归一化(arm64/aarch64、amd64/x86_64 同族,故 arm64 bundle 同时匹配麒麟 aarch64 与 Mac arm64)比对 `uname -m`,不一致 loud-fail(--dry-run 降 info)。**多机 gated off**:真跑多机在 `cmd_deploy` 直接 die,只 `--dry-run` 出计划 + 三条 UNEXERCISED 缝(a backend 端口发布=base compose 只 expose 内网,跨机要 host-published,加 overlay 别改单机 compose 保 dynamic-DNS 路径;b 静态 Caddy upstream=单机 `dynamic a{name backend}` 靠 docker DNS,跨机无共享 DNS 要生成 `k1:8000 k2:8000`;c per-host `.env` DB/Redis 指 infra 机非 localhost)——un-gate = 删那句 die,前提两台真机 deploy 绿。**rollback**:镜像不删,`.fleet-state` 记 current/previous,回退=旧 `AF_VERSION` 重 up。**Mac 验**:`bash -n` 绿;单机 dry-run(load→release gate→`--profile infra` up→wait→smoke 计划)、真 preflight(docker/compose/daemon/disk/runsc-info)、status(项目未起报 no project + LB 未绿)、多机 dry-run 出全计划、多机真跑被 gate/checksum 挡、rollback 空 state 报错、双 lb 基数校验——全绿。`fleet.conf`/`.fleet-state`/`.env.<host>` 已 gitignore(提交 `.example`)。**遗留真机验收**见上「下一步」(a)。
