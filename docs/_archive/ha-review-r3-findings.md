...
| DB 瞬断函数级重试 | ✅ | PR8 已修复 |
| **Lease 续租 fail-closed（对齐 compaction）** | ⚠️ 未对齐 | **F-23（本轮，P1）** |
| **执行崩溃 reconciliation** | 延后 | **F-24（本轮评估，defer）** |
| **Producer 崩溃后 stream 快速收敛** | ❌ 缺失 | **F-25（本轮）** |
...
| Redis 集成测试 fixture 过期参数 | 纳入 PR 修复范围（`stream_timeout` → `execution_timeout`） |
...
| 序号 | ID | 问题 | 等级 | 工作量 |
|------|-----|------|------|--------|
| 1 | F-23 | `_renew_loop` 异常分支未对齐 compaction 的 fail-closed 策略 | P1 | 小 — 加失败计数器（~15 行） |
| 2 | F-25 | Producer 崩溃后 stream 假活 30 分钟 | P1 | 中 — `_wrapped` finally 补 close_stream + consumer lease 检查（~40 行） |
...
| PR | 内容 | 性质 | 回归面 |
|----|------|------|--------|
| **PR9** | F-23 + F-25 + Redis 测试 fixture 修复 | 崩溃收敛 + 策略对齐 | 中 — `_renew_loop` 加计数器 + `_wrapped` 补 close_stream + consumer lease 检查 + 测试 fixture |
