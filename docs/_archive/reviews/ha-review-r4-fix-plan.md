...
| P2 | **PR-C** `QUEUED`/`RUNNING` 前半场建模 | ✅ 已修 | 纯控制面改动：`mark_interactive` 移到取得 semaphore 之后（QUEUED→RUNNING 边，对 lease owner 做 **fail-closed CAS**）；**inject 与 cancel 都 gate 在 interactive(RUNNING)**（round-2 收口：drop queued-cancel —— Redis 中介的 cancel 够不到 worker 本地排队态，强撑会撕裂语义）。stream key 维持 `STREAM_TTL_GRACE` 固定 TTL（心跳续期 deferred 非目标，见验收项 2） |
...
| **`QUEUED` 作为显式状态（inject/cancel 正确分流）** | ✅ 已修 | `interactive` 窗口 = RUNNING；inject 与 cancel **都**在 RUNNING 才放行（QUEUED→409，对称）。QUEUED 不可 cancel 是刻意边界（排队是 worker 本地态、Redis 中介 cancel 够不到；round-2 收口），起跑后即可 cancel。`QUEUED→RUNNING` 由 fail-closed CAS 守 lease 单写 |
...
| `QUEUED → (advance/abort)` | `_wrapped` CAS / heartbeat | **as-built（round-2）**：用户 cancel 不作用于 QUEUED（gate 在 interactive==RUNNING，返回 409）。排队轮要么 advance 到 RUNNING（CAS 守 lease），要么被 abort/fence → ORPHANED。〔此行原草案为「直接 task.cancel()」，已废〕 |
...
3. **`mark_interactive` 对 lease owner 做 fail-closed CAS**：仍持 lease 才标记 + 启动引擎；丢 lease 或 CAS 抛异常（归属不可知）一律 abort（本轮 ORPHANED，响亮可见），绝不跑成静默第二写者。这是 lease 单写不变量的「起跑前前置闸」，独立于 cancel。
4. **cancel 不作用于 QUEUED**（round-2 收口）：cancel gate 在 interactive(RUNNING)、与 inject 对称，排队轮 409、不置任何 flag。round-1 曾让 cancel gate 在 lease + 协作式 flag 以支持 queued-cancel，但「把 Redis 中介的 cancel 续命跨越 worker 本地排队等待」反复撕裂 cancel 语义（r4 round-1/2 同形状 bug）；排队轮无害/瞬态/很快起跑，起跑后即可 cancel（best-effort 契约）。
...
**范围（as-built，纯控制面）**：`api/services/execution_runner.py`（mark_interactive 移位 + fail-closed CAS abort）、`api/services/redis_runtime_store.py` + `runtime_store.py`（`mark_engine_interactive` 改 lease-owner CAS、返回 bool）、`api/routers/chat.py`（cancel gate 在 interactive、QUEUED 给清晰 409）、`docs/architecture/{execution-lifecycle,concurrency,streaming}.md`、测试（runner QUEUED/RUNNING 分离 + fail-closed abort + CAS 归属；路由 QUEUED 分流）。**传输层、`config.py`、前端未改动** —— cancel 复用既有 RUNNING flag 路径，前端复用既有 cancelled 渲染（仅需后续处理 cancel-while-queued 的 409）。
...
7. **（round-2，supersedes #3/#5）PR-C 是否跨层支持 queued-cancel** → **不支持**。cancel gate 回到 interactive(RUNNING)、与 inject 对称；QUEUED 返回 409、不置 flag。根因：QUEUED 是纯 worker 本地的 semaphore 等待，Redis 看不到；让 Redis 中介的 cancel 作用于它就得把 cancel flag 跨「Redis 观察不到的等待」续命 + 与 worker dequeue 对齐 —— 正是 r4 round-1/2 反复同形状 bug（flag 过期、CAS 覆盖、续期与心跳耦合）的根因。排队轮无害/瞬态/很快起跑，起跑后即可 cancel（best-effort 契约）。删除 cancel-flag 续期。
8. **（round-2）PR-C 的 `QUEUED→RUNNING` CAS 异常路径 fail-open 还是 fail-closed** → **fail-closed**。归属不可知时 abort（本轮 ORPHANED，响亮可见、可刷新恢复）优于 fail-open 跑成静默第二写者（破坏 lease 单写不变量，远比丢一个 turn 严重）；redis-py client 已对瞬断重试，仍抛即视为持续故障。
