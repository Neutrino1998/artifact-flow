...
| 优先级 | PR | 状态 | 说明 |
|---|---|---|---|
| P1 | **PR-A** Redis 层 Cluster-safety + hygiene | 待修 | 跨实体 `MGET` → fan-out（`mget_nonatomic` / GET pipeline）；`push_event` 首次 `XADD`→`EXPIRE` 改 pipeline 消孤儿窗口；顺带修 r3 记录的过期测试 fixture |
| P1 | **PR-B** 执行生命周期状态机文档 + `TIMED_OUT` 终态 | 待修 | 写下模型（enum + transition 表 + 不变量 + 测试矩阵）；timeout 改由 controller 经既有 dispatcher 产出唯一终态，SSE 只转发 |
...
| **Redis 访问 Cluster-safety** | ⚠️ 待修 | 数据分析区是 Cluster，`list_active_executions` 跨 slot（PR-A） |
| **超时终态唯一 authority** | ⚠️ 待修 | 当前 SSE error 与 DB cancelled/complete 可分裂（PR-B） |
| **`TIMED_OUT` 作为一等终态** | ⚠️ 待修 | 当前无此终态，超时被记为 CANCELLED / 被 adopt 成 COMPLETE（PR-B） |
...
## PR-A：Redis 层 Cluster-safety + hygiene（P1）
...
**修法**（择一，倾向前者）：
- `mget_nonatomic`：Cluster 客户端内部按 slot 拆分再合并，一次调用，语义不变。
- 单 `GET` 的 pipeline：redis-py 按节点路由，非原子，多几次往返。lease 集合受 `MAX_CONCURRENT` 上界约束，batch 很小，开销可忽略。
...
**修法**：把 `XADD` + `EXPIRE` 放进一个 pipeline。两个操作都作用于同一个 `stream_key`（同 slot），pipeline 安全。低危但 trivial，和 A-1 同属 Redis 层 hygiene，一并修。

### A-3 测试缺口（顺带）

r3 记录：`test_redis_runtime_store.py:62-68` fixture 仍用已过期的 `stream_timeout` 参数（现签名 `execution_timeout`），说明该文件自 PR7 重命名后未被执行过——**多实例 Redis 路径目前无可靠回归保护**。借本 PR 入这个测试文件之机修掉 fixture，并为 A-1/A-2 补 Cluster-safe 断言（至少校验不发跨 slot 多 key 命令）。

**范围**：`redis_runtime_store.py`、`redis_stream_transport.py`、`tests/test_redis_runtime_store.py`。约束写入项目惯例（见下方「约束沉淀」）。

---

## PR-B：执行生命周期状态机文档 + `TIMED_OUT` 终态（P1）
...
1. `TIMED_OUT` 用一等终态（推荐，利于可观测「多少 turn 超时」）还是 `cancel_source="timeout"` 变体（diff 同等大小）。
2. PR-B 超时只裹 `execute_loop` 还是也兜 post-processing（见 B-2 决策点）。
3. PR-C 取消 QUEUED turn 是否写一条 `CANCELLED`/`reason="cancelled_while_queued"` 审计事件，还是「从未跑过 → 不落任何 event」。
4. PR-A 选 `mget_nonatomic` 还是 GET pipeline（取决于实际用的 redis-py Cluster 客户端类型）。
