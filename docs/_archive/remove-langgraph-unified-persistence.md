...
> 状态：已实施（commit bc7dbdc）。本文档已与实现对齐。待完成项：Compaction Phase 1/2、execution_metrics 持久化到 Message.metadata。
...
Message (现有，扩展)
  ...
  ├── (message_id = PK)    消息 ID 同时作为执行标识（实现时决定复用 message.id，不另加字段）
...
| `permission_request` | 需要用户确认 | `{permission_level, tool, params}` |
| `permission_result` | 用户确认结果 | `{approved, tool}` |
| `complete` | 整个请求执行完成 | `{response, execution_metrics}` |
...
执行 loop（内存）
  → tool_b 需要 CONFIRM
  → 推 permission_request 事件到 StreamManager（SSE 通知前端）
...
**幂等性：TaskManager 去重。** 用户连点两次 approve，coroutine 已经在跑（第一次 approve 后 event 已 set），第二次 API 请求被 TaskManager 通过 `message_id` 拒绝，返回 `409 Conflict`。
...
#7   permission_request    {tool: "web_fetch", params: {url: "..."}}
     ← coroutine 暂停（内存 await）
#8   permission_result   {approved: true, always_allow: true}
...
#15  complete   {response: "...", execution_metrics: {...}}
      ...
      → while not completed:
          ...
          → 解析工具调用列表，串行执行每个工具：
              ...
              → 检查权限（读内存 always_allowed_tools）：
                  ├── AUTO → 直接执行
                  └── CONFIRM → 追加 permission_request → 推 SSE → await（内存暂停）
                      → 用户确认 → 追加 permission_result → 继续
...
async def execute_loop(state, context_manager, emit):
    while not state.completed:
        ...
        # drain 用户追加消息（非阻塞）
        queued = drain_queue(state.message_id)
        if queued:
        ...
        for tool in tool_calls:                   # 串行执行
            ...
            if needs_confirm(tool):               # 权限检查
                emit(permission_request)            # 推 SSE（不写库）
    ...
    emit(complete, metrics)              # execution_metrics 随 complete 事件推送
```
...
轮次 N+1 新消息进来
  ...
  → 正常执行 loop
  → complete 事件带 execution_metrics
```
...
| 现有 | 改为 | 原因 |
|------|------|------|
| `thread_id` | `message_id` | 去掉 LangGraph 后不再有 "thread" 概念。实现时决定直接复用 `message.id` 作为执行标识（1:1 关系），不另加 `execution_id` 字段 |

影响范围：`Message.thread_id` 字段删除、API schema (`chat.py`)、前端 SSE 连接 URL、controller 参数。内部项目同步发版，直接改名，不需要兼容窗口。

---

## 旧数据处理

不做迁移，全部清理。`langgraph.db`（checkpoint 数据）直接删除。Messages 表涉及字段重命名（`thread_id` 删除，复用 `id` 作为执行标识）和新增字段（`content_summary`、`response_summary`、`metadata`），旧数据结构不兼容，连同关联的 Conversations、Artifacts 等一起清空，重建 schema。内部项目无外部用户，一步到位。
...
| **TaskManager 去重** | 内存 `dict` 查 `message_id` | Redis 分布式锁 |
...
- **迁移前（当前）**：用户刷新页面，`asyncio.Queue` 中已消费事件丢失。Interrupt 期间刷新 → 丢失 permission_request 事件，但 coroutine 仍在等待，用户需重新发送消息。
