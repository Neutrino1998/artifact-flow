...
### 错误处理：end turn 不等于阻断下一轮 continuation

错误处理基于两个独立判定：

- **不可恢复 → end turn（loud fail）**：`astream_with_retry` 用尽后仍失败、compaction 失败、agent 不存在等 → engine emit ERROR、`state["error"]=True`、break loop。turn 终止是 loud 的，用户能感知失败并选择 retry / 换思路。
...
- **`Message.response` 是 display 用的 final response 快照**，跟历史构建完全解耦。历史走 events，`Message.response` 写什么都不影响下一轮 LLM 的输入。错误时它是错误消息或 `"An error occurred..."`；不要把已生成的 partial `response_content` 塞进去 —— partial `llm_complete`（例如模型刚说"我要 call tool X"就 compaction 失败）不是 final response，写进 `Message.response` 反而让用户误以为这是答案。
- **Tool 异常被 engine 包成 `ToolResult(success=False)`**（`engine.py:529-531`）而不是 raise，让 LLM 看到失败 result 自行决定下一步 —— 这正是 events-as-truth 的体现，by design，不是"漏 set error"的 bug。
- **ERROR 终态事件不进 LLM messages**：`EventHistory._events_to_messages` 只处理 6 种事件类型（USER_INPUT / SUBAGENT_INSTRUCTION / QUEUED_MESSAGE / LLM_COMPLETE / TOOL_COMPLETE / COMPACTION_SUMMARY），ERROR / COMPLETE / CANCELLED 等终态事件入库但只用于 UI 展示和 replay。所以 controller 把终态升级为 ERROR（如 artifact flush 失败）顶多影响 UI 显示精度，不影响下一轮历史构建。
