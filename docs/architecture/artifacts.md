...
这是有意的权衡（plan 决策 6）：读类操作绕过只会让调用者自己看到稍旧的内容，伤不到别人 → 用前端 UX 锁（流式中隐藏版本选择器 / 下载入口）兜，后端保持纯 DB、宽松、多 worker 安全。对比写 / 执行类（发消息 / cancel / 开分支 / delete）绕过会冲突 → 必须后端 lease（409）/ ownership（404）强制。
...
REST 一致性见上文 [执行期间的 REST API 读取](#执行期间的-rest-api-读取)：所有 GET 纯 DB，turn 中落后于 live；`versions[]` / `raw`（二进制原件下载）同样 DB-only，未 flush 的中间版本 / 本轮刚上传的 blob 返回 404。前端在流式执行期间隐藏版本选择器和下载入口规避滞后。
