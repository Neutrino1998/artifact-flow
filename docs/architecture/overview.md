...
| Agent | 职责 | 工具 | 备注 |
|-------|------|------|------|
| `lead_agent` | 协调者，任务规划，Artifact 管理，可直接执行小规模搜索/抓取 | 全部 artifact 工具（含 `grep_artifact`）+ `web_search` (AUTO) + `web_fetch` (CONFIRM) + `call_subagent` | 唯一出口 |
| `research_agent` | 大型知识探索 / 多源整合（隔离上下文） | artifact 工具（含 `grep_artifact`）+ `web_search` (AUTO) + `web_fetch` (CONFIRM) | max 50 rounds |
