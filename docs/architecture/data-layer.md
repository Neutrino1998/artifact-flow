...
erDiagram
    ...
    Message {
        ...
        json metadata "always_allowed_tools / metrics / uploaded_files"
    }
    ArtifactSession {
...
- `metadata` 承载异质字段：`always_allowed_tools` / `execution_metrics` 汇总（内嵌 `last_input_tokens`）/ `uploaded_files`（本轮上传文件 `[{id, filename}]` 的 display 快照，供用户气泡重载后渲染附件；best-effort —— flush 失败的轮次不写）
