...
| 工具 | 类 | 默认权限 | 参数 | 说明 |
|------|---|---------|------|------|
| `web_search` | `WebSearchTool` | AUTO | `query` (string), `max_results` (integer, 默认 10) | Bocha AI API 搜索 |
| `web_fetch` | `WebFetchTool` | CONFIRM | `url` (string), `max_content_length` (integer) | Jina Reader 深度内容提取 |
| `create_artifact` | `CreateArtifactTool` | AUTO | `id`, `title`, `content_type`, `content`, `source` | 创建 Artifact（写入内存缓存） |
| `update_artifact` | `UpdateArtifactTool` | AUTO | `id`, `content`, `partial` (boolean) | 更新 Artifact（partial=true 时追加合并） |
| `rewrite_artifact` | `RewriteArtifactTool` | AUTO | `id`, `content`, `content_type` | 完整替换 Artifact 内容 |
| `read_artifact` | `ReadArtifactTool` | AUTO | `id` | 读取 Artifact 内容 |
