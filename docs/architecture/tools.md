...
**CDATA-aware 拆分** → **标准解析** → **修复后重试** → **诚实错误 ToolCall**
...
避免单个工具调用一次性把上下文撑爆——引擎在每次 `tool_result.success=True` 后检查 `len(result.data) > tool.max_result_size_chars`（`BaseTool` 默认 `50000` 字符），命中即调用 `ArtifactService.persist_tool_result()`：

1. 全文写入新 artifact，ID 形如 `tool_<sanitized_name>_<12hex>`，`source="tool"`，`content_type="text/plain"`
2. 用 `make_preview_slice()` 把回填给模型的 `tool_result.data` 替换为 `<artifact_slice>` 预览（前 `TOOL_PERSIST_PREVIEW_LENGTH=1000` 字符）+ `hint`，告知模型用 `read_artifact(id='...')` 取全文
3. `metadata` 补 `persisted_artifact_id` / `original_size_chars`，前端 SSE 可拿到完整审计信息

设计要点：

- **关掉机制的工具**：`max_result_size_chars=math.inf` 永不落盘。`read_artifact` 必须用 `inf`，否则会形成 Read→artifact→Read 循环
- **Fail-open**：落盘异常时回退到原结果，**不阻断** tool 调用流程（manager 缺失 / DB 异常都走这条路径）
- **失败的 `ToolResult` 不落盘**：仅 `success=True` 且超长才触发；错误信息保留原样
- **不替代 compaction**：本机制处理"单工具产物太大"，compaction 处理"对话历史累计太长"；两者正交（见 [engine.md → Compaction 机制](engine.md#compaction-机制) 关于"不兜底工具结果溢出"的设计权衡）
...
`metadata` 通过 `TOOL_COMPLETE` 事件推送给前端、LLM 看不到（如权限、`update_artifact` 的模糊匹配诊断）。注意：执行期间的实时 Artifact 更新**不再走这里**——`ArtifactService` 在内存更新后直接发独立的 `ARTIFACT_CREATED` / `ARTIFACT_UPDATED` SSE 事件（见 [artifacts.md → SSE 实时推送](artifacts.md#sse-实时推送)），`tool_complete` 不再附带 artifact 快照。
...
| `bash` | `BashTool` | AUTO | `command` (string) | 在本轮沙盒容器内执行 bash；安全边界由沙盒 containment 提供。详见 [sandbox.md](sandbox.md) |
...
保留名列表：`create_artifact`, `update_artifact`, `rewrite_artifact`, `read_artifact`, `grep_artifact`。
