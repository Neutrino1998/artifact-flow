...
**权限处理流程**（在 `engine.py` 的 `_execute_tools` 中）：
...
全局工具在 `dependencies.py` 的 `_load_tools()` 中加载，请求级工具（artifact 操作）在 `_create_controller()` 中合并：
...
与 `xml_parser.py` 互为 formatter / parser 对。提供两个模块级函数：
