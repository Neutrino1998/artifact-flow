...
`cache_salt_field` 是模型 alias 级开关。vLLM 与 vLLM-Ascend 使用同一个上游
OpenAI-compatible 请求结构，字段名都是 `cache_salt`；部署时仍需确认所用镜像内置的
vLLM 版本已支持该字段，且 GPUStack 等中间网关会原样透传。配置后，每次普通 Agent、子 Agent 和
...
服务级 `ARTIFACTFLOW_COMPACTION_RESERVE_TOKENS` 默认为 `20000`；内网 preset 会在
target-local `control/.env` 中显式设置为 `40000`。每个 Agent 的自动压缩阈值为：
