...
echo "  $MANIFEST"
echo
echo "Carry to the intranet node: the image tar and verify tar. The gVisor package"
echo "is only needed for a new host or an explicit runsc upgrade. Then docker load"
echo "the image, tar xzf the verify tar, and run verify/run-all.sh under runsc."
