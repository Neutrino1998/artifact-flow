...
- **Lease-fencing cancel persists events too**: When `_renew_loop` calls `task.cancel()` on the outer `_wrapped` task (`src/api/services/execution_runner.py`), the `CancelledError` propagates through `stream_execute`'s finally, which explicitly cancels the inner `engine_task` so `run_engine`'s `except CancelledError` branch (`src/core/controller.py`) runs `_persist_events` *inside the engine_task* before re-raising. This preserves the "events persist unconditionally" invariant for external cancels (lease fencing, shutdown, hard outer cancel). The cooperative cancel path (`hooks.check_cancelled` polled inside `execute_loop`) sets `state["cancelled"]` and returns normally — the outer post-processing handles it via the success-shaped flow. Both paths write a `CANCELLED` terminal event; the data field differs (`reason="external_cancel"` for the fencing path).
