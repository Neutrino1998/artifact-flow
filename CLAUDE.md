...
- **Error sanitization & `request_id` are separate read-boundary concerns**: `MessageEvent` stores the **raw** error text (event sourcing = full audit; DEBUG replay must show devs the real error). Sanitization (`error` → `"Internal server error"`) is DEBUG-gated at **both** user-facing read boundaries — live SSE push **and** replay — so reload behaves like live; **admin observability endpoints are deliberately NOT sanitized**. The locators `request_id` **and `instance_id`** are **frozen into the error event's `data` at creation** (`request_id` inherits the originating POST's id via the `asyncio.create_task` contextvar copy; `instance_id` names the instance that executed the turn), NOT injected at the read boundary where they would resolve to the *replay GET's* request/instance — the wrong locator. File logs carry the four-tuple `[instance_id|request_id|conv_id|message_id]` and land in per-instance subdirs (`data/logs/<instance_id>/`, obs jsonl likewise — multi-writer rotate clobber fixed by construction); `message_id`/`conv_id` are the bridge to the admin monitor + `MessageEvent` and are never dropped. `instance_id` = container hostname minted once in `utils/instance.py`, also exposed as the `X-Instance-ID` response header and the Redis lease `owner` sidecar key (same hash tag/slot as the lease).
