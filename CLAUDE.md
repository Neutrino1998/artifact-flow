...
- **Cancel paths preserve "events persist unconditionally"**: Three distinct cancel paths exist in `src/core/controller.py` + `engine.py`; all of them land events in the DB, but only the cooperative path runs full post-processing (artifact flush + Message.response update).
  1. **Cooperative cancel** (user clicks cancel → `RuntimeStore.request_cancel` → `hooks.check_cancelled` polled in `execute_loop`): sets `state["cancelled"]=True`, `execute_loop` returns normally, post-processing's success-shaped flow appends a `CANCELLED` terminal (no `reason` field) and runs `_persist_events` + `flush_all` + `update_response`.
  2. **External cancel during execute_loop** (lease fencing `_renew_loop` / shutdown → `task.cancel()` on outer `_wrapped`): `CancelledError` propagates through `stream_execute`'s yield-loop finally, which calls `engine_task.cancel()` so `run_engine`'s `except CancelledError` branch runs `_persist_events` *inside engine_task* (it's a separate Task, so the cancel is consumed once and subsequent awaits work normally). Terminal is `CANCELLED` with `reason="external_cancel"`. Artifact flush + Message.response update are **skipped** — `CancelledError` propagates past post-processing.
  3. **External cancel during post-processing** (engine already returned; cancel lands in `_on_engine_exit` / `exists_async` / `flush_all` / `_persist_events`): an outer `except CancelledError` around post-processing late-persists once via `_persist_events`. If a terminal was already appended (engine semantically completed; cancel only hit infrastructure), it's preserved as-is; otherwise a `CANCELLED` with `reason="external_cancel_post_processing"` is added. Critical idempotency note: only append the new terminal when no terminal exists yet — `MessageEventRepository.batch_create`'s "all-or-nothing duplicate-skip" shortcut breaks if a partially-written batch is retried with one fresh event mixed in.
