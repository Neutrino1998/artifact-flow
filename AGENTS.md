...
- **Code comments describe the current contract, not implementation history.** Keep comments short and locally understandable. Do not make production code depend on archived design documents, historical roadmap phase labels, reviewer numbers, changelog dates, or unnamed “decision N” references. Preserve the durable reason or invariant in plain language; use an active architecture document only when the constraint is genuinely non-local.
