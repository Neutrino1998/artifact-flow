...
- **Error sanitization & locators** (mechanics: `docs/architecture/observability.md`): `MessageEvent` stores raw errors; user-facing SSE and replay sanitize unless DEBUG, while admin observability stays raw. Freeze both `request_id` and `instance_id` into the error event at creation, not at the read boundary. Logs carry `[instance_id|request_id|conv_id|message_id]`; `instance_id` also appears in `X-Instance-ID` and Redis lease ownership.
...
- **Fleet releases are immutable app/config/deploy units.** Target-local mutable state (`deploy/.env`, certs, maintenance flags, autoheal marker, fleet topology/state) stays under the stable control-plane `deploy/`; shipped `config + deploy` lives under `.artifactflow/releases/<release-id>` and `.artifactflow/current` changes only after reconcile + health succeeds. `rollback` switches the whole unit, never only `AF_VERSION`. Full-release identity hashes the app/config/deploy tars; config-only identity includes the inherited active release id, so an id cannot be reused against different content/base. Hotfix bundles also carry an enforced `Expected base release`; retained bundles fail instead of silently rebasing after the active release changes. All release-changing commands are serialized for their full staging→activation→state window by the designated control host's `.artifactflow/fleet-mutation.lock`; a crash leaves a fail-closed stale lock for explicit operator recovery. A config-only release inherits the active deploy unit and app image version, so it does not build/load an app tar. Multi-host currently publishes one backend/frontend per app host and therefore enforces `scale=1` per multi-host app row until per-replica port discovery exists; every multi-host app row has an explicit `advertise=` data-plane address rather than reusing the ssh/`local` transport token.
