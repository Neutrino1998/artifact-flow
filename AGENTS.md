...
- **Sandbox recovery is empty, one-shot, and never replayed** (`src/tools/builtin/sandbox_session.py`). Only an already-started container that later fails at runtime may attempt recovery; initial admission/start and resource-limit failures remain sticky. The failed command is never replayed, and successful recovery must disclose that the previous workspace was lost. Pre-recovery model invocations are fenced by `model_invocation_epoch`.
...
- **Code comments describe the current contract, not implementation history.** Keep comments short and locally understandable. Do not make production code depend on archived design documents, phase labels, reviewer numbers, changelog dates, or unnamed “decision N” references. Preserve the durable reason or invariant in plain language; use an active architecture document only when the constraint is genuinely non-local.
