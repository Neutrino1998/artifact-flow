...
- **Fleet releases are immutable app/config/deploy units.** Target-local mutable state (`deploy/.env`, certs, maintenance flags, autoheal marker, fleet topology/state) stays under the stable control-plane `deploy/`; shipped `config + deploy` lives under `.artifactflow/releases/<release-id>` and `.artifactflow/current` changes only after reconcile + health succeeds. `rollback` switches the whole unit, never only `AF_VERSION`. A config-only release inherits the active deploy unit and app image version, so it does not build/load an app tar. Multi-host currently publishes one backend/frontend per app host and therefore enforces `scale=1` per multi-host app row until per-replica port discovery exists.
