...
- **Conversation delete serializes with execution** (`src/api/routers/chat.py` + `src/core/controller.py`): User single/bulk delete must acquire the same conversation lease as `ExecutionRunner` before the DB DELETE; an existing QUEUED/RUNNING owner means `409` / per-item `active_execution`. This is an acquire-as-mutex operation, never a read-then-delete check (which has a send/delete TOCTOU). Defense in depth: a controller given an existing `conversation_id` must use `create_if_missing=False` both at setup and message insertion. If an admin hard-delete cascade or out-of-band writer removes the row, abort the turn; never recreate it with `user_id=None` as an invisible orphan. Post-processing's exists/FK fail-soft remains for deletion after engine start.
