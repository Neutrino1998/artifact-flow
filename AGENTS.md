...
- **In-engine compaction** (`src/core/compaction_runner.py`): Runs in-loop after each LLM call once tokens exceed `COMPACTION_TOKEN_THRESHOLD`; the per-`agent_name` `compaction_summary` becomes `EventHistory`'s right-to-left boundary, so everything before it is invisible to that agent. Non-obvious rules:
